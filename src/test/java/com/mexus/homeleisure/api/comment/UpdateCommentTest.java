...
class UpdateCommentTest extends BaseControllerTest {
    ...
    void updateCommentSuccess() throws Exception {
        ...
        UpdateCommentRequest updateCommentRequest = UpdateCommentRequest.builder()
                .message("댓글 수정 테스트")
                .build();
        this.mockMvc.perform(RestDocumentationRequestBuilders.put("/trainings/{trainingId}/{commentId}", training
    ...
    void updateCommentFailBecauseTrainingNotExist() throws Exception {
        UpdateCommentRequest updateCommentRequest = UpdateCommentRequest.builder()
                .message("댓글 수정 테스트")
                .build();
        this.mockMvc.perform(RestDocumentationRequestBuilders.put("/trainings/{trainingId}/{commentId}", 1, 2)
                .contentType(MediaType.APPLICATION_JSON)
                .content(this.objectMapper.writeValueAsString(updateCommentRequest)))
